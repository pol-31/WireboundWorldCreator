#include "UiRenderer__Deprecated.h"

#include "../io/Camera.h"

UiRenderer__Deprecated::UiRenderer__Deprecated(GlfwContext& global_glfw_data_,
                                               TileRenderer& tile_renderer,
                                               const Camera* camera)
    : render_data_(global_glfw_data_),
      mdl_manager_(render_data_),
      ui_debugger_(render_data_.vbo_ui_, render_data_.vbo_ui_transform_,
      ...
      terrain_(render_data_, ui_slots_, windows_, text_renderer_,
               ...
               ui_config_window_),
      water_(render_data_, ui_slots_, windows_, text_renderer_, ui_edit_slots_,
             ui_edit_1_, ui_edit_2_, ui_config_window_, mdl_manager_),
      biomes_(render_data_, ui_slots_, windows_, text_renderer_, ui_edit_slots_,
              ui_edit_2_, mdl_manager_),
      objects_(render_data_, ui_slots_, windows_, text_renderer_,
               ui_edit_slots_, ui_edit_2_, mdl_manager_),
      placement_(render_data_, ui_slots_, windows_, ui_edit_slots_, ui_edit_2_,
                 mdl_manager_),
      tiles_(render_data_, windows_),
      player_(render_data_, windows_, text_renderer_, tile_renderer.cur_tile_,
              mdl_manager_),
      ui_menu_(render_data_, text_renderer_, windows_, &terrain_, &water_,
               &placement_, &objects_, &biomes_, &tiles_, &player_, cur_mode_),
...
void UiRenderer__Deprecated::Render(TileRenderer* tile_renderer) {
...
void UiRenderer__Deprecated::RenderPicking(TileRenderer* tile_renderer) {
...
void UiRenderer__Deprecated::Press(int key, int action) {
  ui_buttons_.Press(key, action);
}

void UiRenderer__Deprecated::Parse() {
...
void UiRenderer__Deprecated::Serialize() {
  render_data_.glfw_context_.tile_renderer->cur_tile_.Serialize("");
}

void UiRenderer__Deprecated::Init() {
...
void UiRenderer__Deprecated::SetupGlobalData() {
  ...
  render_data_.glfw_context_.render_data = &render_data_;
...
void UiRenderer__Deprecated::RenderWorldOrigin(glm::vec4 position,
                                               glm::vec4 color) {
  ui_world_origin_.Render(position, color);
}

void UiRenderer__Deprecated::RenderAxis(float scale) {
