...
Ocean::Ocean(const Paths& paths, const OceanTraits& traits)
...
void Ocean::DeInit() {
  ...
  GLuint textures[] = {
    tex_init_spectrum_, tex_precomputed_data_, tex_displacement_,
    tex_derivatives_, tex_turbulence_
  };
...
void Ocean::Init() {
  ...
  layers_scales_ = glm::vec3{traits_.near.scale, traits_.mid.scale, traits_.far.scale};
...
void Ocean::InitTexArray(GLuint* id) {
  ...
  glTexStorage3D(
      GL_TEXTURE_2D_ARRAY, mipLevels, GL_RGBA32F,
      size_, size_, layers
  );
...
void Ocean::OceanTraitsToGpu() const {
  std::array<SpectrumSettings, 3> settings = {
      GenSpectrumSettings(traits_.near),
...
void Ocean::CalculateInitials(
    int cascade_id, float length_scale, float cutoff_low, float cutoff_high) {
  ...
  glBindImageTexture(shader::kOceanSpectrumPrecomputedData, tex_precomputed_data_, 0, false, cascade_id, GL_READ_ONLY, GL_RGBA32F);
  glBindImageTexture(shader::kOceanSpectrumData, tex_init_spectrum_, 0, false, cascade_id, GL_READ_ONLY, GL_RGBA32F);
  glDispatchCompute(size_ / 8, size_ / 8, 1);
  glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
#ifndef NDEBUG
  UnBindImageTexture(shader::kOceanSpectrumBuffer,
                     buffer_tex_, GL_READ_WRITE);
  glBindImageTexture(shader::kOceanSpectrumPrecomputedData, 0, 0, false, cascade_id, GL_READ_ONLY, GL_RGBA32F);
  glBindImageTexture(shader::kOceanSpectrumData, 0, 0, false, cascade_id, GL_READ_ONLY, GL_RGBA32F);
#endif
}
...
void Ocean::UnPackIfftData(int cascade_id, float lambda) {
  using namespace utility;
  glUniform1f(shader::kOceanLambda, lambda);
  ...
  glBindImageTexture(shader::kOceanDisplacement, tex_displacement_, 0, false, cascade_id, GL_WRITE_ONLY, GL_RGBA32F);
  glBindImageTexture(shader::kOceanDerivatives, tex_derivatives_, 0, false, cascade_id, GL_WRITE_ONLY, GL_RGBA32F);
  glBindImageTexture(shader::kOceanTurbulence, tex_turbulence_, 0, false, cascade_id, GL_READ_WRITE, GL_RGBA32F);
  ...
  glBindImageTexture(shader::kOceanDisplacement, 0, 0, false, cascade_id, GL_WRITE_ONLY, GL_RGBA32F);
  glBindImageTexture(shader::kOceanDerivatives, 0, 0, false, cascade_id, GL_WRITE_ONLY, GL_RGBA32F);
  glBindImageTexture(shader::kOceanTurbulence, 0, 0, false, cascade_id, GL_READ_WRITE, GL_RGBA32F);
#endif
}
