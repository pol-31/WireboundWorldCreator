 ...
 * have the same properties (density, render approach (full/half), shading,
 * etc);
 ...
 * - because we use the same std::vector<...>positions, we can also keep it
 * sorted like this: 1 2 3 4 5 6 7 8 9 0, so then we can skip simply as i*=2 or
 * i*=4 or i*=8 and get: 1, 3, 5, 7, 9 OR 1, 5, 9, etc...
 * */

std::vector<Grass::Blade> Grass::GenerateBlades(
    const std::vector<int>& points, const std::vector<uint8_t>& heights) {
  ...
  for (auto id : points) {
    for (int i = 0; i < 10; ++i) {
      const auto x =
          static_cast<float>(id & 1023) / 16.0f - 32.0f - 1 + dis(gen) * 0.5f;
      const auto y =
          static_cast<float>(id >> 10) / 16.0f - 32.0f - 1 + dis(gen) * 0.5f;
      ...
      blades.push_back({glm::vec4(x, z, y, orientation_dis(gen)),
...
Grass::Grass()
    : grass_shader_(/*"../shaders/Grass.vert", "../shaders/Grass.tesc",
...
void Grass::UpdateAnimation(float delta_time) {
  ...
  glUniform1f(1, static_cast<float>(delta_time / 1e3f));  // TODO: what's this
  glUniform1f(2, 1000.0f /*wind_magnitude_*/);
  glUniform1f(3, wind_wave_length_);
  glUniform1f(4, wind_wave_period_);

  /*
   * tile 1024x1024 subdivided on n tiles, where n depends on distance to
   * camera; if we in the centre of the tile, we draw 9 tiles with max res, so
  ...
  /*for (int i = 0; i < Tile::kSubTilesNum; ++i) {
    ...
    glDispatchCompute(blades_num, 1, 1); // do we need diff buffers or ...TODO:
  POSITIONS?
  }*/
}
