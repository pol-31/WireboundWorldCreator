...
void UiTextInput::Render() {
  ...
  text_renderer_.RenderText(
      sp_text_, text_input_, 1.0f, glm::vec2{0.0f},
...
void UiTextInput::RenderBack() { sp_back_.Render(); }
...
void UiTextInput::Press() { text_renderer_.StartInput(this); }
...
void UiTextInput::SetText(std::string_view text) { text_input_ = text; }
...
void UiTextInput::SetScissorArea() {
  ...
  auto x_start = int((GetLeftBorder() + 1) * 0.5f * gWindowWidth);
  auto x_length =
...
void UiTextLabelBase::UpdateTransform() { sp_text_.UpdateTransform(); }
...
void UiText::Render(std::string_view text, float scale, glm::vec2 translate,
                    TextRenderer::Alignment alignment) {
  text_renderer_.RenderText(sp_text_, text, scale, translate, alignment);
}

void UiText::RenderPicking(std::string_view text, float scale,
                           glm::vec2 translate,
                           TextRenderer::Alignment alignment) {
