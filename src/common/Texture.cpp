...
TextureUi::TextureUi(std::string_view path, GLint format,
                     int filter, int wrap)
    : Texture() {
  format_ = format;
  glGenTextures(1, &opengl_id_);
...
Texture32F::Texture32F(std::string_view path, GLint format)
    : Texture() {
  format_ = format;
  glGenTextures(1, &opengl_id_);
  glBindTexture(GL_TEXTURE_2D, opengl_id_);
...
TextureUi::TextureUi(int width, int height, int format,
                     int filter, int wrap)
    : Texture() {
  width_ = width;
  height_ = height;
...
Texture32F::Texture32F(int width, int height, int format)
    : Texture() {
  width_ = width;
  height_ = height;
  format_ = format;
...
void Texture::CreateTextureData() {
  glTexImage2D(GL_TEXTURE_2D, 0, format_, width_,
               height_, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
}

void TextureUi::CreateTextureData() {
  glTexImage2D(GL_TEXTURE_2D, 0, format_, width_,
               height_, 0, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, nullptr);
}
...
void Texture::Store(std::string_view path, GLint channels,
                    GLint format, int component) const {
...
void Texture32F::CreateTextureData() {
  glTexImage2D(GL_TEXTURE_2D, 0, format_, width_,
               height_, 0, GL_RGBA, GL_FLOAT, nullptr);
}
...
void Texture32F::LoadTextureData(std::string_view path) {
  ...
  size_t num_floats = file_size / sizeof(float);32;
...
void Texture32F::GetTextureData(
...
std::vector<uint8_t> Texture32F::FloatsToUint(
