...
void MouseTransform::UpdateStartAngle() {
  ...
  auto view = global_data.camera->GetViewMatrix(map_scale);
...
void MouseTransform::TranslateSelected(
...
//  auto map_scale = 1.0f; //TODO: ????
  ...
  *scale_ = glm::clamp(
      prev_value * (glm::vec3(1.0f) - axis_ + axis_ * value), glm::vec3(-200.0f), glm::vec3(200.0f));
...
glm::vec3 MouseTransform::GetInstanceTransform() {
  return *translate_;
}
...
void MouseTransform::TranslateSelectedVerticesUp(
  ...
  if (value_y == 0.0f || falloff == 0.0f) {
    return;
  }
  std::cout << "move on " << value_y
            << " with the falloff " << falloff << std::endl;
  vertices_transform_shader_.Bind();
  utility::BindImageTexture(
      0, ui_shared_resources_.global_glfw_callback_data_
             .tile_renderer->cur_tile_.map_terrain_height, GL_READ_WRITE);
