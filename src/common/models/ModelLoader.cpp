...
void ModelLoader::LoadScene(std::string_view collisions_path,
  ...
  scene_ = std::make_unique<Scene>();
  {
    ...
    glBufferSubData(GL_ARRAY_BUFFER, pos_offset,     pos_size,     data.all_positions.data());
    glBufferSubData(GL_ARRAY_BUFFER, norm_offset,    norm_size,    data.all_normals.data());
    glBufferSubData(GL_ARRAY_BUFFER, uv_offset,      uv_size,      data.all_uvs.data());
    glBufferSubData(GL_ARRAY_BUFFER, tangent_offset, tangent_size, data.all_tangents.data());
  ...
  glBufferSubData(GL_ARRAY_BUFFER, tangent_offset, tangent_size, data_rigged.all_tangents.data());
    ...
    std::vector<std::uint8_t>& out_data)
{
  ...
  const auto& accessor = model.accessors.at(primitive.attributes.at(name.data()));
  const auto& bufferView = model.bufferViews.at(accessor.bufferView);
  const auto& buffer = model.buffers.at(bufferView.buffer);

  // 2. Calculate pointers and sizes
  const uint8_t* startPtr = &buffer.data[bufferView.byteOffset + accessor.byteOffset];
...
tinygltf::Model ModelLoader::LoadBufferMerge(std::string_view path,
  ...
  for (const auto& mesh : model.meshes) {
    ...
    for (const auto& primitive : mesh.primitives) {
      const auto& posAccessor = LoadBufferSafely("POSITION", model, primitive, data.all_positions);
      ...
      if (is_rigged) {
        ...
        auto jointsAccessor = LoadBufferSafely("JOINTS_0", model, primitive, data.all_joints);
...
void ModelLoader::LoadMaterials(std::string_view path, tinygltf::Model& model,
  ...
  for (const auto& m : model.materials) {
    ...
    if (normal_tex_id == -1) {
      std::cerr << "mesh doesn't have a normal texture" << std::endl;
      normal_tex_id = 0;
    } else {
