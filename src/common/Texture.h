...
class Texture {
 public:
  ...
  Texture(GLuint opengl_id, GLsizei width, GLsizei height, GLint format)
  ...
  virtual void GetTextureData(GLint format, std::vector<uint8_t>& pixels,
  ...
  [[nodiscard]] GLsizei GetFormat() const { return format_; }
  ...
  [[nodiscard]] GLsizei GetSizePixels() const { return width_ * height_; }

  explicit operator bool() const { return opengl_id_ != 0; }
...
class Texture32F final : public Texture {
 public:
  ...
  Texture32F(int size, GLint format) : Texture32F(size, size, format) {}

  void CreateTextureData() override;

  void LoadTextureData(std::string_view path) override;

  void GetTextureData(GLint format, std::vector<uint8_t>& pixels, int component,
                      GLint channels) const override;
...
class TextureUi final : public Texture {
 public:
  ...
  TextureUi(std::string_view path, int format, int filter = GL_LINEAR,
            int wrap = GL_REPEAT);

  TextureUi(int width, int height, GLint format, int filter = GL_LINEAR,
            int wrap = GL_REPEAT);
