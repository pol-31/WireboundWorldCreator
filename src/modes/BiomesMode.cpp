...
BiomesMode::BiomesMode(SharedResources& shared_resources)
    : IEditMode(shared_resources) {}

void BiomesMode::Render() {
  shared_resources_.tile_renderer_.Render();
}

void BiomesMode::RenderPicking() {
  shared_resources_.tile_renderer_.RenderPickingTerrain();
}

void BiomesMode::BindCallbacks() {
  glfwSetScrollCallback(gWindow, BiomesModeScrollCallback);
  glfwSetMouseButtonCallback(gWindow, BiomesModeMouseButtonCallback);
  //  glfwSetKeyCallback(gWindow, TerrainModeKeyCallback);
  glfwSetKeyCallback(gWindow, WasdKeyCallback);
}

int BiomesMode::Hover(std::uint32_t global_id) {
  return -1;
}
