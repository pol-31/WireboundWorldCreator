...
UiEditOcean::UiEditOcean(
    ...
    : Base({data::VboIdMain::kWaterEditDesk}, 1.0f,
      ...
      color_palette_(
          {data::VboIdMain::kWaterColorPaletteHS},
          {data::VboIdMain::kWaterColorColorCursor}, glm::vec2{1.0f}),
      color_brightness_(
          {data::VboIdMain::kWaterColorPaletteB},
      ...
      ui_event_handler_({
          &pin_,
          &ocean_layer_config_.btn_config_, &ocean_layer_config_.tg_visible_,
...
void UiEditOcean::SetInstanceId(int id) {
  ...
  ->water.SetWaterColor(base_instances_[selected_id_].color);
}
...
void UiEditOcean::RenderOceanConfig() {
  ...
  float entry_height = sprite_.GetHeight() / (ocean_layers_.size() + 5); // +pads
  for (int i = 0; i < ocean_layers_.size(); ++i) {
    ocean_layer_config_.Render(
        ocean_layers_[i].GetVisible(), next_offset, ocean_layers_[i].GetTextId());
...
UiEditRiver::UiEditRiver(
    ...
    : Base({data::VboIdMain::kWaterEditDesk}, 1.0f,
      ...
      ui_event_handler_({
          &pin_, &river_layer_config_.sl_strength_,
          &accept_, &name_, &color_palette_,
          &color_brightness_, &random_generate_
      }),
      ui_shared_resources_(ui_shared_resources),
...
void UiEditRiver::RenderRiverConfig() {
  ...
  river_layer_config_.Render(
      instances_[selected_id_].transparency, next_offset,
      river_layer_config_.pressed_strength_id_ == 0, data::TextId::kStrength);
  next_offset.y -= entry_height;
  river_layer_config_.Render(
      instances_[selected_id_].viscosity, next_offset,
      river_layer_config_.pressed_strength_id_ == 1, data::TextId::kSwell);
}
