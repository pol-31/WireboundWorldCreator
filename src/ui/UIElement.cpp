...
void UIElement::StopAnimation(UiAnimationType type)
{
	for (int i = (int)mAnimations.size() - 1; i >= 0; --i)
		if (mAnimations[i]->GetType() == UiAnimationType::Slide)
		{
...
void UIElement::AutoLayout()
{
	for (UIElement *e : mChildren)
	{
		...
		if (e->IsVisible())
		{
			mWidth.Set(std::max(GetWidth(), e->GetX() + e->GetWidth() - GetX() + e->GetPaddingRight()), PIXELS);
			mHeight.Set(std::max(GetHeight(), e->GetY() + e->GetHeight() - GetY() + e->GetPaddingBottom()), PIXELS);
		}
	}
}
