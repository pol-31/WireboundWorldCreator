...
public:
	/// Constructor
							...
							// UITexturedQuad(int inX, int inY, int inWidth, int inHeight) : mX(inX), mY(inY), mWidth(inWidth), mHeight(inHeight) { }
							// UITexturedQuad(const Texture *inTexture, int inX, int inY, int inWidth, int inHeight, int inInnerX, int inInnerY, int inInnerWidth, int inInnerHeight) : mX(inX), mY(inY), mWidth(inWidth), mHeight(inHeight), mInnerX(inInnerX), mInnerY(inInnerY), mInnerWidth(inInnerWidth), mInnerHeight(inInnerHeight) { }
  ...
  //TODO: we're going to use BUCKETS instead, so not to rebind Texture for each
  //TODO: we're going to use BUCKETS instead, so not to rebind Texture for each
  //TODO: we're going to use BUCKETS instead, so not to rebind Texture for each

  // AND WE KEEP THE INNER-OUTER THING, i LIKE IT
