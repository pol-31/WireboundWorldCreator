...
void UISlider::Draw() const
{
	...
	GetManager()->DrawQuad(slider_start, GetY() + sy, slider_end - slider_start, mSlider.mHeight, mSlider, JPH::Color::sWhite);

	// Draw thumb
	int tx = GetThumbStart(slider_start, slider_end);
	int ty = (GetHeight() - mThumb.mHeight) / 2;
	GetManager()->DrawQuad(tx, GetY() + ty, mThumb.mWidth, mThumb.mHeight, mThumb, JPH::Color::sWhite);
}
...
void UISlider::SetValueInternal(float inValue)
{
	...
	mCurrentValue = std::clamp(mMinValue + step * mStepValue, mMinValue, mMaxValue);
...
void UISlider::UpdateStaticText()
{
	if (mStaticText != nullptr)
	{
		...
		{
			// Integer values
			mStaticText->SetText(std::to_string(int(std::round(mCurrentValue))));
		}
		else
		{
